...
//QuickSelect
			...
			for (var i in AgentHelper.Druids) {
				if (AgentSystem.HasAgent(AgentHelper.Druids[i][0])) {
					...
					if (Agent.m_Level == 50) {
						...
						if (!m_Controller.settingQuickselectName) AgentButton = new QuickSelectButton(AgentHelper.Druids[i][1])
	...
	private function __AgentButtonClicked(button:QuickSelectButton) {
		var id = AgentHelper.GetSwitchAgent(button.AgentData.m_AgentId, m_Controller.settingRealSlot, 0);
		if (id){
			AgentHelper.SwitchToAgent(button.AgentData.m_AgentId, m_Controller.settingRealSlot);
		}
